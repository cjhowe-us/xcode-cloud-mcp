...
import type { CiBuildRun, CiBuildAction, StartBuildRunRequest } from '../types.js';
...
export class BuildsClient extends BaseAPIClient {
   ...
   */
  async start(workflowId: string, gitReferenceId?: string): Promise<CiBuildRun> {
    ...
    const response = await this.post<CiBuildRun>('/v1/ciBuildRuns', requestBody);
   ...
   */
  async listForWorkflow(workflowId: string, options?: { limit?: number }): Promise<CiBuildRun[]> {
  ...
  async getActions(buildRunId: string): Promise<CiBuildAction[]> {
    const response = await this.get<CiBuildAction[]>(`/v1/ciBuildRuns/${buildRunId}/actions`);
    return response.data;
  }
}
